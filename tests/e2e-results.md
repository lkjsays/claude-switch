...
- [x] `-p`·stdin 파이프·종료코드 — **자동 테스트가 실계정 없이 이미 검증**
      (`tests/integration.bats:186` 인자 전달, `:199` `--` 없는 전달, `:209` 종료코드 전달,
      `:215` stdin 파이프, `:236` 토큰 미노출). 스텁 기반이지만 래퍼의 계약을 직접 검사함.
...
- [x] `Ctrl-C` 전달 확인 — **1회로는 종료되지 않고 2회에 종료됨**

`Ctrl-C` 1회가 무반응인 것은 `claude`의 오탈출 방지 가드이며 `claude-switch`의 결함이
아닙니다. 오히려 **시그널이 claude에 직접 도달했다는 증거**입니다(래퍼가 시그널을
삼켰다면 아무 반응도 없었을 것). `exec`로 프로세스를 교체하므로 래퍼는 시그널 경로에
존재하지 않습니다.

프로세스 잔존 검사: PPID=1 인 고아 `claude` **없음**. `claude-switch` 프로세스도
존재하지 않음(`exec` 교체). → `e2e.md:93`의 "1회로 종료된다"는 서술을 실제 동작에 맞게
정정했습니다.
...
| 인터랙티브 TTY 정상 | ✅ B4 |
| `-p`·stdin·종료코드 | ✅ 자동 테스트 (integration.bats) |
| `Ctrl-C` 전달 + 고아 프로세스 없음 | ✅ B4 (2회 입력은 claude 자체 가드) |
...
## 릴리스 판정 — ✅ v2.0.0 릴리스 (2026-08-19)

**커밋 `3b6b325`, 태그 `v2.0.0`, CI success.**

합격 기준의 모든 항목을 통과했거나, 통과 불가능한 항목을 문서화된 제약으로 전환했습니다.

### 의도적으로 수행하지 않은 항목

| 항목 | 이유 |
|---|---|
| **B5 `--isolate` 실계정 실행** | 합격 기준 항목이 아님. 필요 시 `claude-switch run --isolate <profile> -- -p OK` 한 줄로 확인 가능 |
| **B6 새 토큰 등록 경로** | 합격 기준의 "브라우저 승인 화면 확인"은 프로필 최초 등록 시점(2026-08-09)에 이뤄졌고, 비표시 입력 경로 자체는 `tests/integration.bats:155-164`가 검증함 |

**B6를 수행하지 않았으므로 revoke 대상 테스트 토큰이 없습니다.** `e2e.md` 합격 기준의
"테스트 토큰 revoke 완료" 항목은 해당 없음입니다.

### 이 E2E가 잡아낸 것

자동 테스트 166개는 `tests/stubs/claude` 스텁으로 돌기 때문에 **실제 CLI 출력 형식을
검증하지 못합니다.** 그 결과 아무도 측정하지 않은 문서 주장 4개가 모두 틀린 상태로
남아 있었고, 이번 절차가 전부 잡아냈습니다:

1. `verify`가 계정·조직을 표시한다 → 표시하지 않음 (`CHANGELOG.md`, `e2e.md` §3·§5)
2. 확인 라벨이 `계정 확인` → 실제는 `인증 상태 :` (`e2e.md` §3)
3. `.credentials.json` mtime이 불변이어야 한다 → 공식 CLI가 토큰 갱신 시 재작성 (`e2e.md` §10)
4. `Ctrl-C` 1회로 종료된다 → claude 자체 가드로 2회 필요 (`e2e.md` §4)

전부 같은 유형(관측 없이 쓴 서술)이며, 소스 코드에는 결함이 없었습니다.
