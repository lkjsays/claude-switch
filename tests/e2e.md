...
- `Ctrl-C`가 claude에게 전달된다. **`claude` 자체가 오탈출 방지로 2회 입력을 요구하므로
  1회로는 종료되지 않는 것이 정상이다.** 1회 입력에 claude가 반응하고 2회에 종료되면
  시그널이 래퍼를 건너 claude에 직접 도달한 것이다(`exec` 교체의 결과).
- 종료 후 `ps -eo pid,ppid,args | grep '[c]laude'` 에 PPID=1 인 고아 프로세스가 없다.
  `claude-switch` 는 `exec` 로 자신을 교체하므로 래퍼 프로세스 자체가 존재하지 않는다.
- 종료 후 셸로 정상 복귀한다.
